%Green and red emitted fluorescence signals were filtered and 
%split to separate photodetectors and digitally sampled at 6100 Hz via 
%a data acquisition board (National Instruments, Model # NI USB-6211). 
 

%Extract peaks with an effective sampling rate of 211 Hz, and correct the 
%signal for photobleaching by fitting the decay with a double exponential.

%The signals in the green channel are transformed back to absolute 
%fluorescence and DF/F is computed.

channelData=state.photoreceiver.channelData(6101:end,:);
sample_rate = 211;
figure;plot(channelData(:,1),'g');hold on 
plot(channelData(:,2),'r');
channelData1=findpeaks(channelData(:,1),'MinPeakDistance',20);
channelData2=findpeaks(channelData(:,2),'MinPeakDistance',20);
